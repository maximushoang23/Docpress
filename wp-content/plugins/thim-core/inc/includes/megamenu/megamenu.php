 ...
 * Class Mega_Menu
 *
 * Create mega menu for wordpress theme, This class base on the "Max Mega Menu" plugin version 2.2.3.1. Thanks Tom Hemsley.
 * License: GPL-2.0+
 */
...
if ( ! class_exists( 'Mega_Menu' ) ) :
	...
	final class Mega_Menu {
		...
		public function __construct() {
			...
			add_filter( 'megamenu_nav_menu_objects_after', array(
				$this,
			...
			add_filter( 'conditional_menus_theme_location', array(
				$this,
			...
			add_shortcode( 'menu', array( $this, 'register_shortcode' ) );
			add_shortcode( 'megamenu', array( $this, 'register_shortcode' ) );
		...
		public function register_shortcode( $atts ) {
			...
			return "<!-- menu not found [megamenu location={$atts['location']}] -->";
		...
		private function define_constants() {
			...
			define( 'MEGAMENU_BASE_URL', THIM_CORE_INC_URI . '/includes/megamenu/' );
			define( 'MEGAMENU_PATH', THIM_CORE_INC_PATH . '/includes/megamenu/' );
		...
		public function modify_nav_menu_args( $args ) {
			...
			if ( isset ( $settings[ $current_theme_location ]['enabled'] ) && $settings[ $current_theme_location ]['enabled'] == true ) {
				...
				$sanitized_location = str_replace( apply_filters( "megamenu_location_replacements", array(
					"-",
					" "
				...
				$defaults = array(
					'menu'        => $menu_id,
					'container'   => false,
					'menu_class'  => 'mega-menu mega-menu-horizontal navbar',
		...
		public function admin_notices() {
			...
			if ( ! $this->is_compatible_wordpress_version() ) :
				...
				?>
				<div class="error">
					<p><?php _e( 'Mega Menu is not compatible with your version of WordPress. Please upgrade WordPress to the latest version', 'megamenu' ); ?></p>
				</div>
				<?php
