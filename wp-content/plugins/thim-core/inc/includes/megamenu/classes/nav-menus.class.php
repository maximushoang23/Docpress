	...
	class Mega_Menu_Nav_Menus {
		...
		public function __construct() {
			...
			if ( function_exists( 'siteorigin_panels_admin_enqueue_scripts' ) ) {
				add_action( 'admin_print_scripts-nav-menus.php', array(
					$this,
					'siteorigin_panels_admin_enqueue_scripts'
				) );
			}

			if ( function_exists( 'siteorigin_panels_admin_enqueue_styles' ) ) {
				add_action( 'admin_print_styles-nav-menus.php', array(
					$this,
					'siteorigin_panels_admin_enqueue_styles'
				) );
			}
		...
		public function register_nav_meta_box() {
			...
			if ( 'nav-menus.php' == $pagenow ) {
				...
				add_meta_box( $class, __( "Mega Menu Settings", "thim-core" ), array( $this, 'metabox_contents' ), 'nav-menus', 'side', 'high' );
		...
		public function enqueue_menu_page_scripts( $hook ) {
			...
			wp_localize_script( 'mega-menu', 'megamenu', array(
					'debug_launched'     => __( "Launched for Menu ID", "thim-core" ),
					'get_started'        => __( "Use these settings to enable Mega Menu", "thim-core" ),
					'launch_lightbox'    => __( "Mega Menu", "thim-core" ),
					'is_disabled_error'  => __( "Please enable Mega Menu using the settings on the left of this page.", "thim-core" ),
					'saving'             => __( "Saving", "thim-core" ),
					'nonce'              => wp_create_nonce( 'megamenu_edit' ),
					'nonce_check_failed' => __( "Oops. Something went wrong. Please reload the page.", "thim-core" )
				) );
		...
		public function save() {
			...
			if ( isset( $_POST['menu'] ) && $_POST['menu'] > 0 && is_nav_menu( $_POST['menu'] ) && isset( $_POST['megamenu_meta'] ) ) {
				...
				$default = addslashes( json_encode( array(
					array(
						'name'  => 'megamenu_meta[primary][enabled]',
						'value' => '0'
					)
				) ) );

				$raw_submitted_settings = ( '[]' != $_POST['megamenu_meta'] ) ? $_POST['megamenu_meta'] : $default;
		...
		public function print_enable_megamenu_options( $menu_id ) {
			...
			if ( ! count( $theme_locations ) ) {
				...
				echo "<p>" . __( "This theme does not register any menu locations.", "thim-core" ) . "</p>";
				echo "<p>" . __( "You will need to create a new menu location and use the Max Mega Menu widget or shortcode to display the menu on your site.", "thim-core" ) . "</p>";

			} else if ( ! count( $tagged_menu_locations ) ) {

				echo "<p>" . __( "Please assign this menu to a theme location to enable the Mega Menu settings.", "thim-core" ) . "</p>";
		...
		public function settings_table( $location, $settings ) {
			...
			<table>
				<tr>
					<td><?php _e( "Enable", "thim-core" ) ?></td>
					<td>
						<input type='checkbox' class='megamenu_enabled' name='megamenu_meta[<?php echo $location ?>][enabled]'
						       value='1' <?php checked( isset( $settings[ $location ]['enabled'] ) && $settings[ $location ]['enabled'] == 1 ); ?> />
					</td>
				</tr>
