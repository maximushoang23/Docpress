...
if ( ! class_exists( 'Mega_Menu_Menu_Item_Manager' ) ) :
	...
	class Mega_Menu_Menu_Item_Manager {
		...
		public function add_mega_menu_tab( $tabs, $menu_item_id, $menu_id, $menu_item_depth, $menu_item_meta ) {
			...
			if ( $menu_item_depth > 0 ) {
				$tabs['mega_menu'] = array(
					...
					'content' => '<em>' . __( "Mega Menus can only be created on top level menu items.", "thim-core" ) . '</em>'
				);
			...
			$submenu_options = apply_filters( "megamenu_submenu_options", array(
				'flyout'   => __( "Flyout Menu", "thim-core" ),
				'megamenu' => __( "Mega Menu", "thim-core" )
			), $menu_item_meta );

			$return = "<label for='mm_enable_mega_menu'>" . __( "Sub menu display mode", "thim-core" ) . "</label>";
			...
			$return .= "    <option value='1' " . selected( $menu_item_meta['panel_columns'], 1, false ) . ">1 " . __( "column", "thim-core" ) . "</option>";
			$return .= "    <option value='2' " . selected( $menu_item_meta['panel_columns'], 2, false ) . ">2 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "    <option value='3' " . selected( $menu_item_meta['panel_columns'], 3, false ) . ">3 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "    <option value='4' " . selected( $menu_item_meta['panel_columns'], 4, false ) . ">4 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "    <option value='5' " . selected( $menu_item_meta['panel_columns'], 5, false ) . ">5 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "    <option value='6' " . selected( $menu_item_meta['panel_columns'], 6, false ) . ">6 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "    <option value='7' " . selected( $menu_item_meta['panel_columns'], 7, false ) . ">7 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "    <option value='8' " . selected( $menu_item_meta['panel_columns'], 8, false ) . ">8 " . __( "columns", "thim-core" ) . "</option>";
			$return .= "</select>";

			$return .= "<select id='mm_widget_selector'>";
			$return .= "    <option value='disabled'>" . __( "Select a Widget to add to the panel", "thim-core" ) . "</option>";
			...
			if ( count( $items ) ) {
				...
				foreach ( $items as $item ) {
					...
					$return .= '            <a class="widget-option widget-contract" title="' . esc_attr( __( "Contract", "thim-core" ) ) . '"></a>';
					...
					$return .= '            <a class="widget-option widget-expand" title="' . esc_attr( __( "Expand", "thim-core" ) ) . '"></a>';
					$return .= '            <a class="widget-option widget-action" title="' . esc_attr( __( "Edit", "thim-core" ) ) . '"></a>';
					$return .= '        </div>';
		...
		public function add_general_settings_tab( $tabs, $menu_item_id, $menu_id, $menu_item_depth, $menu_item_meta ) {
			...
			$return .= '    <h4 class="first">' . __( "Menu Item Settings", "thim-core" ) . '</h4>';
			$return .= '    <table>';
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Hide Text", "thim-core" );
			$return .= '            </td>';
			...
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Hide Arrow", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';
			...
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Disable Link", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';
			...
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Hide Item on Mobile", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';
			...
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Hide Item on Desktop", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';
			...
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Menu Item Align", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';

			if ( $menu_item_depth == 0 ) {
				$return .= '            <select name="settings[item_align]">';
				$return .= '                <option value="left" ' . selected( $menu_item_meta['item_align'], 'left', false ) . '>' . __( "Left", "thim-core" ) . '</option>';
				$return .= '                <option value="right" ' . selected( $menu_item_meta['item_align'], 'right', false ) . '>' . __( "Right", "thim-core" ) . '</option>';
				$return .= '            </select>';
				$return .= '            <div class="mega-description">';
				$return .= __( "Right aligned items will appear in reverse order on the right hand side of the menu bar", "thim-core" );
				$return .= '            </div>';
			} else {
				$return .= '<em>' . __( "Option only available for top level menu items", "thim-core" ) . '</em>';
			}
			...
			$return .= '    <h4>' . __( "Sub Menu Settings", "thim-core" ) . '</h4>';

			$return .= '    <table>';
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Sub Menu Align", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';

			if ( $menu_item_depth == 0 ) {
				$return .= '            <select name="settings[align]">';
				$return .= '                <option value="bottom-left" ' . selected( $menu_item_meta['align'], 'bottom-left', false ) . '>' . __( "Left edge of Parent", "thim-core" ) . '</option>';
				$return .= '                <option value="bottom-right" ' . selected( $menu_item_meta['align'], 'bottom-right', false ) . '>' . __( "Right edge of Parent", "thim-core" ) . '</option>';
				$return .= '            </select>';
				$return .= '            <div class="mega-description">';
				$return .= __( "Right aligned flyout menus will expand to the left", "thim-core" );
				$return .= '            </div>';
			} else {
				$return .= '<em>' . __( "Option only available for top level menu items", "thim-core" ) . '</em>';
			}
			...
			$return .= '        <tr>';
			$return .= '            <td class="mega-name">';
			$return .= __( "Hide Sub Menu on Mobile", "thim-core" );
			$return .= '            </td>';
			$return .= '            <td class="mega-value">';
		...
		public function add_icon_tab( $tabs, $menu_item_id, $menu_id, $menu_item_depth, $menu_item_meta ) {
			...
			$return = "<h4 class='first'>" . __( "Menu Item Icon", "thim-core" ) . "</h4>";
			...
			$return .= "<input type='text' class='filter_icons' placeholder='" . __( "Search", "thim-core" ) . "' /><div class='clear'></div>";
