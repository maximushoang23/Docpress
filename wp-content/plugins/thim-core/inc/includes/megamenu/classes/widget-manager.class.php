	...
	class Mega_Menu_Widget_Manager {
		...
		public function register_sidebar() {
			...
			register_sidebar( array(
				'id'          => 'mega-menu',
				'name'        => __( "Mega Menu Widgets", "thim-core" ),
				'description' => __( "Do not manually edit this area.", "thim-core" )
			) );
		}
		...
		public function ajax_save_widget() {
			...
			if ( $saved ) {
				$this->send_json_success( sprintf( __( "Saved %s", "thim-core" ), $id_base ) );
			} else {
				$this->send_json_error( sprintf( __( "Failed to save %s", "thim-core" ), $id_base ) );
			}
		...
		public function ajax_update_widget_columns() {
			...
			if ( $updated ) {
				$this->send_json_success( sprintf( __( "Updated %s (new columns: %d)", "thim-core" ), $widget_id, $columns ) );
			} else {
				$this->send_json_error( sprintf( __( "Failed to update %s", "thim-core" ), $widget_id ) );
			}
		...
		public function ajax_update_menu_item_columns() {
			...
			if ( $updated ) {
				$this->send_json_success( sprintf( __( "Updated %s (new columns: %d)", "thim-core" ), $id, $columns ) );
			} else {
				$this->send_json_error( sprintf( __( "Failed to update %s", "thim-core" ), $id ) );
			}
		...
		public function ajax_add_widget() {
			...
			if ( $added ) {
				...
				$this->send_json_error( sprintf( __( "Failed to add %s to %d", "thim-core" ), $id_base, $menu_item_id ) );
			}
		...
		public function ajax_delete_widget() {
			...
			if ( $deleted ) {
				$this->send_json_success( sprintf( __( "Deleted %s", "thim-core" ), $widget_id ) );
			} else {
				$this->send_json_error( sprintf( __( "Failed to delete %s", "thim-core" ), $widget_id ) );
			}
		...
		public function ajax_reorder_items() {
			...
			if ( $moved ) {
				$this->send_json_success( sprintf( __( "Moved (%s)", "thim-core" ), json_encode( $items ) ) );
			} else {
				$this->send_json_error( sprintf( __( "Didn't move items", "thim-core" ), json_encode( $items ) ) );
			}
		...
		public function show_widget( $id ) {
			...
			$params = array_merge( array( array_merge( array( 'widget_id' => $id, 'widget_name' => $wp_registered_widgets[ $id ]['name'] ) ) ), (array) $wp_registered_widgets[ $id ]['params'] );
			...
			<div class='widget-content'>
				<form method='post'>
					...
					<div class='widget-controls'>
						<a class='delete' href='#delete'><?php _e( "Delete", "thim-core" ); ?></a> |
						<a class='close' href='#close'><?php _e( "Close", "thim-core" ); ?></a>
					</div>
		...
		public function update_menu_item_columns( $menu_item_id, $columns ) {
			...
			return update_post_meta( $menu_item_id, '_megamenu', $settings );
		}
		...
		public function update_widget_order( $widget_id, $order, $parent_menu_item_id ) {
			...
			return update_option( 'widget_' . $id_base, $current_widgets );
		}
		...
		public function update_menu_item_order( $menu_item_id, $order, $parent_menu_item_id ) {
			...
			return update_post_meta( $menu_item_id, '_megamenu', $submitted_settings );
		}
