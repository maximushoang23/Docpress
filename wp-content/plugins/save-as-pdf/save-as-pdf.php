...
/**
 * Plugin Name: Save as PDF Button
...
class Restpack
{
  ...
  function __construct()
  {
    ...
    add_action('wp_ajax_nopriv_restpack_ajax', array(&$this, 'restpack_ajax'));

    if(isset($_REQUEST['action']) && $_REQUEST['action']=='restpack_ajax'){
      do_action( 'wp_ajax_restpack_ajax' );
      do_action( 'wp_ajax_nopriv_restpack_ajax');
    }
  ...
  {
    if (strpos($file, 'save-as-pdf.php') !== false) {
      $settings_link = [
        'demo' => '<a href="https://restpack.io/html2pdf" target="_blank">Demo</a>',
        'documentation' => '<a href="https://restpack.io/html2pdf/save-as-pdf-instructions" target="_blank">Documentation</a>'
      ];
  ...
  function init()
  {
    ...
    add_settings_field($this->key("width"), 'Width', array(&$this, 'render_settings_field'), __FILE__, $third_section, array(
      "key" => "width"
    ));
    ...
    add_settings_field(
      ...
      array(
        "key" => "http_headers",
        "placeholder" => "Authorization: Basic ZWx1c3Vhcmlv\\nCookie: wordpress_1234=admin"
      )
    );
  }
  ...
  function shortcode($attrs)
  {
    ...
    $_props = array(
      ...
      "filename" => implode([get_the_title(), '.pdf']),
    ...
    return "<a class=\"restpack-button\"  href=\"https://restpack.io/html2pdf/save-as-pdf?" . $query . "\" data-props=" . json_encode($props) .">
      <img style='display: inline; width:" . $options['button_width'] . "' src='" . plugin_dir_url(__FILE__) . 'assets/' . $options['button_icon'] . "' alt='" . $button_text . "' title='" . $button_text . "'/> " . $button_text . "</a>";
   }
  ...
  function restpack_ajax()
  {
    ...
    if ($response->errors) {
      echo json_encode(array('errors' => $response->errors));
      return wp_die();
    }
