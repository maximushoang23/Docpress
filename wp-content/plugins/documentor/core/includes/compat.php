...
if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	add_filter( 'woocommerce_product_tabs', 'doc_woo_new_product_tab' );
	function doc_woo_new_product_tab( $tabs ) {
		global $post;
		$pid = $post->ID; 
		if( !empty( $pid ) ) {
			...
			if( !empty( $attachid ) ) {
				...
				'priority'  => 50,
				'callback'  => 'doc_woo_new_product_tab_content'
				);
			}
		}
		return $tabs;
	}
	function doc_woo_new_product_tab_content() {
		// The new tab content
		global $post;
		$pid = $post->ID;
		$attachid = get_post_meta( $pid, '_documentor_attachid', true ); 
...
add_action( 'init', 'doc_load_tablepress_in_the_admin', 11 );
function doc_load_tablepress_in_the_admin() {
...
add_filter('guide_html', 'doc_add_crayons_to_guide_html');
add_filter('section_html', 'doc_add_crayons_to_guide_html');
function doc_add_crayons_to_guide_html($content) {
