<?php 
if( !class_exists( 'DocumentorFonts' ) ) {
	class DocumentorFonts {
		...
		function get_google_fonts() {
			...
			$fonts = array();
			$json  = array();
			$fontsfurl = Documentor::documentor_plugin_url( 'core/includes/webfonts.json' );
		...
		function get_google_fonts_html($name, $gid, $current_value) {
			...
			$html='<select name="'.esc_attr($name).'" id="'.esc_attr($gid).'" class="google-fonts" autocomplete="off">
				<option value="">'. __( '&mdash; Google Fonts &mdash;', 'documentor' ).'</option>
				<!-- Google Serif -->';
				 if ( ! empty( $documentor_google_subsets['serif'] ) ):
					$html.='<optgroup label="'. __( 'Google Serif Fonts', 'documentor' ).'" class="google_label">';
			...
			if ( ! empty( $documentor_google_subsets['sans-serif'] ) ): 
				$html.='<optgroup label="'. __( 'Google Sans Serif Fonts', 'documentor' ).'" class="google_label">';
			...
			if ( ! empty( $documentor_google_subsets['display'] ) ): 
				$html.='<optgroup label="'. __( 'Google Display Fonts', 'documentor' ).'" class="google_label">';
			...
			if ( ! empty( $documentor_google_subsets['handwriting'] ) ): 
				$html.='<optgroup label="'. __( 'Google Handwriting Fonts', 'documentor' ).'" class="google_label">';
			...
			if ( ! empty( $documentor_google_subsets['monospace'] ) ): 
				$html.='<optgroup label="'. __( 'Google Monospace Fonts', 'documentor' ).'" class="google_label">';
		...
		function get_google_font_subset_html($currfont, $subsetname, $subsetid, $current_value) {
			...
			if(isset($fonts[$currfont])) {
				...
				$html.='<option value="">'. __( '&mdash; Font Subset &mdash;', 'documentor' ).'</option>';
			}
			$html.='</select>';
			return $html;
		}
		...
		public static function load_fontsdiv_callback() {
			...
			if( $type == 'regular' ) {
				...
				<th scope="row">'. __('Font','documentor') .'</th>
				<td>';
				...
				$html.='<span class="moreInfo">
					<div class="tooltip1">'. __('This value will be fallback font if Google web font value is specified below','documentor').'
					</div>
				</span>
				</td>
				</tr></table>';
				...
				$nmgsubset = $nm.'subset';
				$html.='<table><tr valign="top">
				<th scope="row">'. __('Google Web Font','documentor').'</th>
				<td>';
				...
				<tr valign="top">
				<th scope="row">'. __('Google Font Weight','documentor').'</th>
				...
				<tr valign="top">
				<th scope="row">'. __('Google Font Subset','documentor').'</th>
				...
				$html.='<table><tr valign="top">
				<th scope="row">'. __('Custom Font','documentor').'</th>
				<td>';
