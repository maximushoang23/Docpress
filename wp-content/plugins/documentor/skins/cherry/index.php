...
class DocumentorDisplaycherry{
	...
	function buildFrontMenus($obj) {
		...
		foreach( $sectiondata as $secdata ) {
			...
			$liactiveclass = '';
			$icon = '<span class="icon-uniF067 doc-menu-icon doc-icons"></span>';
			if( $secdata->type == 3 ) {
				$html .= '<li class="doc-actli">'.$icon.$mtitle;
			} else {
				//pretty links
				...
				$html .= '<li class="doc-actli">'.$icon.'<a class="documentor-menu" '.$href.' data-section-id="'.esc_attr($secdata->sec_id).'" '.$cssarr['navmenu'].' data-href="'.$linkhref.'">'.$mtitle.'</a>';
			}
	...
	function buildFrontSections($obj, $i, $k, $j) {
		$stylesheetpath = Documentor::documentor_plugin_url( 'skins/cherry/print.css' );
		if(isset($this->docid)) {
			if( class_exists( 'DocumentorSection' ) ) {
				$id = $this->docid;
				$ds = new DocumentorSection( $id, $obj->id);
		...
		if( $ds != null ) {
			...
			foreach( $sectiondata as $secdata ) {	
				...
				if( $secdata->type != 3 ) { //If not a link section
					...
					$shtml.= $starttag.' class="doc-sec-title" '.$cssarr['sectitle'].'> <span class="doc-sec-count" '.$cntcss.'>'.$scnt.'</span>'.$section_title.$endtag;
	...
	function get_section_ajaxcontent( $docid, $secid, $currenturl, $nextsecid, $prevsecid, $nextsecname, $prevsecname ) {
		...
		if( $settings['disable_ajax'] == 1 ) { //if ajax is enabled
			...
			if( $secdata->type != 3 ) { //If section is not a link
				...
				$html.= '<div class="documentor-section '.$tran_class.' section-'.esc_attr($secdata->sec_id).'" id="'.esc_attr($sectionid).'" data-section-id="'.$secdata->sec_id.'">';
				$sec_title = ( !empty( $section_title ) ) ? $section_title : $menu_title;
				$shtml.= $starttag.' class="doc-sec-title" '.$cssarr['sectitle'].'> '.$sec_title.$endtag;
				//front-end edit section
				$shtml .= '<div class="documentor-social doc-noprint">';
				if( post_type_exists($type) ) { 
					if ( is_user_logged_in() && current_user_can('edit_post', $postid)) {
	...
	function display() {
		...
		if( $settings['button'][4] == 1 ) {
			...
			if( $settings['window_print'] == 0 ) {
				wp_enqueue_style( 'doc_'.$settings['skin'].'_printcss', Documentor::documentor_plugin_url( 'skins/cherry/print_style.css' ), false, DOCUMENTOR_VER, 'print');	
			} 
		}
		...
		if( !empty( $guideobj->sections_order ) ) {
			...
			global $doc_style_counter_cherry;
			if( !isset( $doc_style_counter_cherry ) or $doc_style_counter_cherry < 1 ) {
				$html .="<link rel='stylesheet' href='".Documentor::documentor_plugin_url( 'skins/cherry/style.css' )."' type='text/css' media='all' />";
				$doc_style_counter_cherry++;
			}
			//wrap div
					...
					if( $guide->rel_id != 0 ) {
						$html .= '<div class="documentor-related" '.$cssarr['navmenu'].'>';
							...
							foreach ( (array) $menu_items as $key => $menu_item ) {
							    ...
							    $url = $menu_item->url;
							    $html .= '<div>'.$icon.'<a href="' . $url . '" '.$cssarr['navmenu'].' >' . $title . '</a></div>';
							}
							$html .= '</div>';
						$html .= '</div>';
					}	
