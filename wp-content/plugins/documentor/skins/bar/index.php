...
class DocumentorDisplaybar{
	...
	function buildFrontSections($obj, $i, $k, $j) {
		$stylesheetpath = Documentor::documentor_plugin_url( 'skins/bar/print.css' );
		if(isset($this->docid)) {
			if( class_exists( 'DocumentorSection' ) ) {
				$id = $this->docid;
				$ds = new DocumentorSection( $id, $obj->id);
		...
		if( $ds != null ) {
			...
			foreach( $sectiondata as $secdata ) {	
				...
				if( $secdata->type == 3 ) {
					...
					$shtml.='<span class="doc-sec-count" '.$cntcss.'>'.$type.$scnt.'.</span>';
	...
	function display() {
		...
		if( $settings['button'][4] == 1 ) {
			...
			if( $settings['window_print'] == 0 ) {
				wp_enqueue_style( 'doc_'.$settings['skin'].'_printcss', Documentor::documentor_plugin_url( 'skins/bar/print_style.css' ), false, DOCUMENTOR_VER, 'print');	
			} 
		}
		...
		if( !empty( $guideobj->sections_order ) ) {
			...
			global $doc_style_counter_bar;
			if( !isset( $doc_style_counter_bar ) or $doc_style_counter_bar < 1 ) {
				$html .="<link rel='stylesheet' href='".Documentor::documentor_plugin_url( 'skins/bar/style.css' )."' type='text/css' media='all' />";
				$doc_style_counter_bar++;
			}
					...
					if( $guide->rel_id != 0 ) {
						$html .= '<div class="documentor-related" '.$cssarr['navmenu'].'>';
							...
							foreach ( (array) $menu_items as $key => $menu_item ) {
							    ...
							    $url = $menu_item->url;
							    $html .= '<div><span class="icon-uniF0C1 doc-rlink-icon"></span><a href="' . $url . '" '.$cssarr['navmenu'].' >' . $title . '</a></div>';
							}
							$html .= '</div>';
						$html .= '</div>';
					}	
