		...
		foreach( $sectiondata as $secdata ) {
			...
			if( $secdata->type == 3 ) {
				...
				if( $settings['disable_ajax'] == 0 ) { $href = 'href="'.$linkhref.'"'; }
 				$html .= '<li class="doc-actli">';
 				$html .= '<a class="documentor-menu" '.$href.' data-section-id="'.esc_attr($secdata->sec_id).'" '.$cssarr['navmenu'].' data-sec-counter="'.$scnt.'" data-href="'.$linkhref.'">'.$mtitle.'</a>';
			}
	...
	function buildFrontSections($obj, $i, $k, $j) {
		...
		if( $ds != null ) {
			...
			foreach( $sectiondata as $secdata ) {
				...
				$stylesheetpath = Documentor::documentor_plugin_url( 'skins/default/print.css' );
				$shtml = $sectionid = "";
				$postid = $secdata->post_id;
				if( $secdata->type == 0 ) $type = 'documentor-sections';
				else if( $secdata->type == 1 ) $type = 'post';
				...
				if( $secdata->type == 3 ) {
					$sec_title = '';
					$shtml.= $starttag.' class="doc-sec-title" style="display:none;"> ';
				...
				if( $secdata->type != 3 ) { //If not a link section
					...
					$shtml.= $starttag.' class="doc-sec-title" '.$cssarr['sectitle'].'> <span class="doc-sec-count" '.$cntcss.'>'.$scnt.'.</span>'.$sec_title.$endtag;
					...
					if( post_type_exists($type) ) {
						if ( is_user_logged_in() && current_user_can('edit_post', $postid)) {
							...
							$shtml.= '<span class="doc-postedit-link"><a href="'.esc_url($edtlink).'" target="_blank"><i class="fa fa-pencil-square-o"></i></a></span>';
						}
					}
					...
					if( $settings['button'][1] == 1 ) {
						$shtml.= '<span onclick="prompt(\'Press Ctrl + C, then Enter to copy to clipboard\',\''.$currurl.'\')"><i class="fa fa-link"></i></span>';
					}
					if( $settings['button'][2] == 1 ) {
					$subject = $sec_title;
					$shtml.= '<span><a href="mailto:?subject='.$subject.'&body='.$currurl.'"><i class="fa fa-envelope-o"></i></a></span>';
					}
					if( $settings['disable_ajax'] == 1 ) {
						if( $settings['button'][3] == 1 && $secdata->pdf_id != 0 ) {
							...
							if( $attachdata != NULL ) {
								...
								if( $pdfExists ) {
									$shtml.= '<span><form method="post" class="save_docpdf"><span class="save_secpdf_inline"> <img src='.$url3.' > </span><input type="hidden" name="doc_pdf" value="section_pdf" /><input type="hidden" name="secid" value="'.$secdata->sec_id.'"><input type="hidden" name="docid" value="'.esc_attr($this->docid).'"> </form></span>';
						...
						if( $settings['button'][4] == 1 ) {
							...
							$shtml.= '<span><a class="doc-print" data-printspath="'.$stylesheetpath.'"><i class="fa fa-print"></i></span></a></span>';
						}
					}
					$shtml .= '</div>';
				}
				if( $settings['disable_ajax'] == 0 ) {
					$html.= '<div class="'.$parent.' documentor-section '.esc_attr($tran_class).' section-'.esc_attr($secdata->sec_id).'" id="'.esc_attr($sectionid).'" data-section-id="'.esc_attr($secdata->sec_id).'">';
					$html .= $shtml;
					$html .= '<div class="doc-sec-content" '.$cssarr['sectioncontent'].'>';
					if( $secdata->type != 3	) { //not link section
						$post = get_post( $postid );
						if( $post != null ) {
							$pcontent = $post->post_content; // Content
					...
					if( $secdata->type != 3	) { //not link section
						$html.= '<div class="documentor-help">';
								if( $settings['feedbackcnt'] == 1 ) {
									$html .= '<div class="feedback doc-feedbackcnt"><span class="upvote">'.$secdata->upvote.'</span><span class="doc-noprint"><a class="positive-feedback" href="#" ><i class="fa fa-thumbs-up"></i></a></span></div>
									<div class="feedback"><span class="doc-noprint downvote">'.$secdata->downvote.'</span><span class="doc-noprint"><a class="negative-feedback" href="#" ><i class="fa fa-thumbs-down"></i></a></span></div>';
								}
								if( $settings['suggest_edit'] == 1 ) {
									$html .= '<div class="feedback"><span class="doc-noprint"><i class="fa fa-bullhorn"></i><a title="Give suggestion" href="#sugestedit_popup'.$this->docid.'" rel="leanModal" class="spopupopen">'.__("Suggest edit","documentor").'</a></span></div>';
								}
					...
					if( $i == 1 || ( isset( $_REQUEST['section'] ) && !empty( $_REQUEST['section'] ) && $i == 1 ) ) {
						...
						if( $secdata->type != 3	) { //not link section
							$html.= '<div class="documentor-help">';
									...
									if( $settings['suggest_edit'] == 1 ) {
										$html.= '<span class="doc-noprint"><a title="Give suggestion" href="#sugestedit_popup'.$this->docid.'" rel="leanModal" class="spopupopen" >
										<i class="fa fa-comment"></i> '.__("Suggest edit","documentor").' </a></span>';
									}
									if( $settings['updated_date'] == 1 ) {
										$html.='<div class="doc-mdate">'.__('Last updated on','documentor').' '.$modified_date.'</div>';
									}
	...
	function get_section_ajaxcontent( $docid, $secid, $currenturl, $nextsecid, $prevsecid, $nextsecname, $prevsecname ) {
		...
		if( $settings['disable_ajax'] == 1 ) { //if ajax is enabled
			...
			if( $secdata->type != 3 ) { //If section is not a link
				...
				$html.= '<div class="documentor-section '.$tran_class.' section-'.esc_attr($secdata->sec_id).'" id="'.esc_attr($sectionid).'" data-section-id="'.$secdata->sec_id.'">';
				$sec_title = ( !empty( $section_title ) ) ? $section_title : $menu_title;
				$shtml.= $starttag.' class="doc-sec-title" '.$cssarr['sectitle'].'> <span class="doc-sec-count" '.$cntcss.'></span>'.$sec_title.$endtag;
				$shtml .= '<div class="documentor-social doc-noprint">';
				//front-end edit section
				if( post_type_exists($type) ) {
					if ( is_user_logged_in() && current_user_can('edit_post', $postid)) {
						$edtlink = get_edit_post_link($postid);
						$shtml.= '<span class="doc-postedit-link"><a href="'.$edtlink.'" target="_blank"><i class="fa fa-pencil-square-o"></i></a></span>';
					}
				}
				$currurl = '';
				if( !empty( $currenturl ) ) {
				...
				if( $settings['button'][1] == 1 ) {
					$shtml.= '<span onclick="prompt(\'Press Ctrl + C, then Enter to copy to clipboard\',\''.$currurl.'\')"> <img width="15" height="15" src='.$url1.' > </span>';
				}
				if( $settings['button'][2] == 1 ) {
					$subject = $sec_title;
					$shtml.= '<span><a href="mailto:?subject='.$subject.'&body='.$currurl.'"><img src='.$url2.' ></a></span>';
				}
				if( $settings['disable_ajax'] == 1 ) {
					if( $settings['button'][3] == 1 && $secdata->pdf_id != 0 ) {
						$attachdata = get_post( $secdata->pdf_id );
						if( $attachdata != NULL ) {
							$pdfExists=$this->docFileExists( $attachdata->guid );
							if( $pdfExists ) {
								$shtml.= '<span><form method="post" class="save_docpdf"><span class="save_secpdf_inline"> <img src='.$url3.' > </span><input type="hidden" name="doc_pdf" value="section_pdf" /><input type="hidden" name="secid" value="'.esc_attr($secdata->sec_id).'"><input type="hidden" name="docid" value="'.esc_attr($docid).'"></form></span>';
							}
						}
					}
					if( $settings['button'][4] == 1 ) {
			...
			if( $secdata->type != 3	) { //not link section
				$html.= '<div class="documentor-help">';
						...
						if( $settings['suggest_edit'] == 1 ) {
							$html .= '<span class="doc-noprint"><a title="Give suggestion" href="#sugestedit_popup'.$docid.'" rel="leanModal" class="spopupopen" ><i class="fa fa-comment"></i><span> '.__('Suggest edit','documentor').'</span> </a></span>';
						}
						if( $settings['updated_date'] == 1 ) {
							$html.='<div class="doc-mdate">'.__('Last updated on','documentor').' '.$modified_date.'</div>';
						}
						$html .= '<div class="negative-feedbackform doc-noprint"></div>
						<div class="feedback-msg doc-noprint"></div>';
						if( $settings['socialshare'] == 1 && $settings['sbutton_position'] == 'bottom' ) {
							$html .= $guide->get_social_buttons( $settings, $sec_title, $currurl );
						}
	...
	function display() {
		...
		if( $settings['button'][4] == 1 ) {
			...
			if( $settings['window_print'] == 0 ) {
				wp_enqueue_style( 'doc_'.$settings['skin'].'_printcss', Documentor::documentor_plugin_url( 'skins/default/print_style.css' ), false, DOCUMENTOR_VER, 'print');
			}
		}
		...
		if( !empty( $guideobj->sections_order ) ) {
			...
			global $doc_style_counter_default;
			if( !isset( $doc_style_counter_default ) or $doc_style_counter_default < 1 ) {
				$html .="<link rel='stylesheet' href='".Documentor::documentor_plugin_url( 'skins/default/style.css' )."' type='text/css' media='all' />";
				$doc_style_counter_default++;
			}
			...
			if( $settings['disable_ajax'] == 0 ) {
				$pdfurl = Documentor::documentor_plugin_url( $root."/images/pdf.png" );
				$printurl = Documentor::documentor_plugin_url( $root."/images/document-print.png" );
				...
				if( $settings['button'][3] == 1 && $guideobj->pdf_id != 0 ) {
					...
					if( $attachdata != NULL ) {
						$pdfExists=$this->docFileExists( $attachdata->guid );
						if( $pdfExists ) {
							$html.= '<span><form method="post" class="save_docpdf"><span class="save_secpdf"><i class="fa fa-file-pdf-o"></i></span><input type="hidden" name="doc_pdf" value="document_pdf" /><input type="hidden" name="doc_id" value="'.esc_attr($this->docid).'"></form></span>';
						}
					}
				}
				if( $settings['button'][4] == 1 ) {
					$html.= '<a class="doc-print" data-printspath="'.$stylesheetpath.'"><i class="fa fa-print"></i></span></a>';
				}
				$html .= '</span></div>';
			...
			if( !empty( $obj ) ) {
				...
				$html.= apply_filters('before_document_menu', '', get_the_ID());
					...
					if( $guide->rel_id != 0 ) {
						$html .= '<div class="documentor-related" '.$cssarr['navmenu'].'>';
							...
							foreach ( (array) $menu_items as $key => $menu_item ) {
							    ...
							    $url = $menu_item->url;
							    $html .= '<div><a href="' . $url . '" '.$cssarr['navmenu'].'>' . $title . '</a></div>';
							}
							$html .= '</div>';
						$html .= '</div>';
					}
