		...
		foreach( $sectiondata as $secdata ) {
			...
			if( $secdata->type == 3 ) {
				...
				if( $settings['disable_ajax'] == 0 ) { $href = 'href="'.$linkhref.'"'; }
 				...
 				$html .= '<a class="documentor-menu" '.$href.' data-section-id="'.esc_attr($secdata->sec_id).'" '.$cssarr['navmenu'].' data-sec-counter="'.$scnt.'" data-href="'.$linkhref.'" '.$aattr.'>'.$mtitle.'</a>';
			}
	...
	function buildFrontSections($obj, $i, $k, $j) {
		...
		if( $ds != null ) {
			...
			foreach( $sectiondata as $secdata ) {	
				...
				$stylesheetpath = Documentor::documentor_plugin_url( 'skins/broad/print.css' );
				$shtml = $sectionid = "";
				$postid = $secdata->post_id;
				if( $secdata->type == 0 ) $type = 'documentor-sections';
				else if( $secdata->type == 1 ) $type = 'post';
				...
				if( $secdata->type != 3 ) { //If not a link section
					...
					if( $settings['disable_ajax'] == 1 ) {
						...
						if( $settings['button'][4] == 1 ) {
							...
							$shtml.= '<span><a class="doc-print" data-printspath="'.$stylesheetpath.'"> <img src='.$url4.'> </span></a></span>';
						}
					}
					$shtml .= '</div>';
				} 
					...
					if( $secdata->type != 3	) { //not link section
						$html.= '<div class="documentor-help">';
								if( $settings['feedback'] == 1 ) {
									$html.='<span class="doc-noprint"><img src='.$url5.' />'.__("Was this helpful?","documentor").'</span> 
								...
								if( $settings['suggest_edit'] == 1 ) {  
									...
									<span><img height="15" width="15" src='.$editpng.'></span> '.__("Suggest edit","documentor").' </a></span>';
								}
		...
		if( $settings['disable_ajax'] == 1 ) { //if ajax is enabled
			...
			if( $secdata->type != 3	) { //not link section
				$html.= '<div class="documentor-help">';
						...
						if( $settings['suggest_edit'] == 1 ) {    
							$html .= '<span class="doc-noprint"><a title="Give suggestion" href="#sugestedit_popup'.$docid.'" rel="leanModal" class="spopupopen" ><img height="15" width="15" src='.$editpng.'><span> '.__('Suggest edit','documentor').'</span> </a></span>';
						}
						if( $settings['updated_date'] == 1 ) {
							$html.='<div class="doc-mdate">'.__('Last updated on','documentor').' '.$modified_date.'</div>';
						}
	...
	function display() {
		...
		if( $settings['button'][4] == 1 ) {
			...
			if( $settings['window_print'] == 0 ) {
				wp_enqueue_style( 'doc_'.$settings['skin'].'_printcss', Documentor::documentor_plugin_url( 'skins/broad/print_style.css' ), false, DOCUMENTOR_VER, 'print');	
			} 
		}
		...
		if( !empty( $guideobj->sections_order ) ) {
			...
			if( !isset( $doc_style_counter_broad ) or $doc_style_counter_broad < 1 ) {
				$html .="<link rel='stylesheet' href='".Documentor::documentor_plugin_url( 'skins/broad/style.css' )."' type='text/css' media='all' />";
			...
			if( $settings['disable_ajax'] == 0 ) {
				...
				if( $settings['button'][3] == 1 && $guideobj->pdf_id != 0 ) {
					...
					if( $attachdata != NULL ) {
						$pdfExists=$this->docFileExists( $attachdata->guid );
						if( $pdfExists ) {
							$html.= '<span><form method="post" class="save_docpdf"><span class="save_secpdf"> <img height="15" width="15" src='.esc_url($pdfurl).' > </span><input type="hidden" name="doc_pdf" value="document_pdf" /><input type="hidden" name="doc_id" value="'.esc_attr($this->docid).'"></form></span>';
						}
					}
				}
				if( $settings['button'][4] == 1 ) {
					$html.= '<a class="doc-print" data-printspath="'.$stylesheetpath.'"> <img height="15" width="15" src='.$printurl.'> </span></a>';
				}
				$html .= '</span></div>';
			} else {
				if( isset( $settings['search_box'] ) && $settings['search_box'] == '1' ) {
