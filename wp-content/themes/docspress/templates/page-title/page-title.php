					...
					if ( ( is_category() || is_archive() || is_search() || is_404() ) ) {
						...
						if ( is_single() ) {
							...
							if ( get_post_type() == "our_team" ) {
								echo '<' . $typography . '>' . esc_html__( 'Our Team', 'thim-docspress' );
								echo '</' . $typography . '>';
							}
							if ( get_post_type() == "testimonials" ) {
								echo '<' . $typography . '>' . esc_html__( 'Testimonials', 'thim-docspress' );
								echo '</' . $typography . '>';
							}
						} else {
							echo '<' . $typography . '>';
						...
						echo '<h1>';
						echo ( $front_title != '' ) ? $front_title : esc_html__( 'Blog', 'thim-docspress' );
						echo '</h1>';
						echo ( $subtitle != '' ) ? '<div class="banner-description" ' . $c_css_sub_color . '><p>' . $subtitle . '</p></div>' : '';
					}
					?>
